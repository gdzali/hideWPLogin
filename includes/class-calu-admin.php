...
class CALU_Admin {
	...
	public function admin_init() {
		...
		add_settings_section( 'calu-permalink', __( 'Hide WordPress Admin', 'calu' ), array( $this, 'settings' ), 'permalink' );	
	...
	public function admin_notices() {
		...
		if ( ! is_network_admin() && $pagenow === 'options-permalink.php' && isset( $_GET['settings-updated'] ) ) {
			echo '<div class="updated"><p>' . sprintf( __( 'Admin linki %s olarak değiştirildi.', 'calu' ), '<strong><a href="' . calu_login_url() . '">' . calu_login_url() . '</a></strong>' ) . '</p></div>';
		}
	}

	public function settings(){
		?>
		<p>Aşağıya yeni admin linkini girerek değişim işlemini sağlayabilirsiniz.</p>
		...
		<table id="calu-permalink" class="form-table">
			<tbody>
				<tr>
					<th><label for="calu-permalink"><?php _e('Login Link', 'calu'); ?></label></th>
					<td> 
